...
| 7 | Maintenance | `maintenance/run` | `operations.ts` (offers Run in background (`run_async`) and surfaces the returned operation id) | `sigillum api maintenance run` (`--run-async`) | Fully covered. |
...
| 20 | Wallet profiles | `profiles/evm` (+`/upsert`,`/delete`), `profiles/eth-stealth` (+`/upsert`,`/delete`), `profiles/eth-xpub` (+`/upsert`,`/delete`), `profiles/eth-seed` (+`/upsert`,`/create`,`/delete`) — 13 routes | `wallets.ts`, `walletManager.ts`, `journey.ts` | `sigillum api profiles <evm\|stealth\|eth-xpub\|eth-seed> <list\|upsert\|create\|delete>` | Fully covered. Plan task 3.2 (additive): every delete accepts an optional `prune_inventory` flag (CLI `--prune-inventory`; the console's shared confirm dialog offers the cascade as an explicit checkbox) that forgets the profile's scanned history, receive allocations, and counterparty bindings in the same operation — absent/false keeps the legacy orphaning behavior. |
| 21 | Wallet key ops (read/derive, no spend) | `wallets/eth-xpub/export`, `wallets/eth-xpub/derive`, `wallets/eth-stealth/export`, `wallets/eth-stealth/generate`, `wallets/eth-stealth/check` | `wallets.ts` (both exports + derive); generate/check not in UI yet | `sigillum api wallets xpub-export\|xpub-derive\|stealth-export\|stealth-generate\|stealth-check` | No sign/send in this family, so it is safe to script; D1 adds the CLI. Plan task 3.4 (additive): xpub export responses carry a `warning` restating the whole-tree exposure (CLI prints it to stderr; console toasts it, pins it in a warning box, and gates the first xpub copy of each session behind an inform dialog); `eth-xpub/derive` is the documented unauthenticated local derivation oracle (debug-traced). |
...
| 23 | Inventory registry | `inventory/wallets`, `chains` (+`/upsert`,`/delete`), `inventory/chains` (+`/upsert`,`/delete` legacy alias), `inventory/scan/evm`, `inventory/addresses/delete`, `inventory/watch-addresses` (+`/upsert`,`/delete`), `inventory/token-registry` (+`/import`,`/delete`) | `inventory.ts` (+ `journey.ts`, `walletManager.ts`); the scan form offers Run in background (`run_async`) and surfaces the returned operation id | `sigillum api chains <list\|upsert\|delete>`; `sigillum api inventory <list\|chains\|watch\|token-registry\|scan-evm\|prune-addresses>` (`scan-evm --all-configured-chains --probe-token-registry --run-async --partition-providers`) | Fully covered. Plan task 3.2 (additive): `inventory/addresses/delete` forgets scanned-address rows (plus their holdings and per-address block cursors) by AND-combined selectors (`prune-addresses --address\|--wallet-profile\|--provider-profile\|--chain-id\|--account-index`, at least one required); pruning removes history, not derivation — a re-scan re-observes still-derived indices as fresh rows. |
| 24 | Discovery jobs | `discovery/jobs`, `discovery/jobs/cancel`, `discovery/jobs/resume` | `inventory.ts` (cancel gates on the shared confirm dialog; resume restarts in the background; the job list itself is surfaced via the scan flow, not fetched directly) | `sigillum api discovery jobs <list\|cancel\|resume>` | Covered; the UI job-list gap is cosmetic and the CLI lists jobs. Cancel/resume are real since plan task 1.2: cancel cooperatively stops the live scan (409 on terminal jobs), resume starts a new background operation from the job's checkpoints. |
...
| 27 | Treasury | `treasury/overview`, `treasury/policy` (+`/update`), `treasury/receive-addresses` (+`/allocate`,`/rotate`,`/purge`), `treasury/parties` (GET+POST) (+`/update`,`/delete`) — 10 registrations | `treasury.ts` (+ `journey.ts`, `walletManager.ts`, `setup.ts` policy update) | `sigillum api treasury <overview\|policy\|policy-update\|receive-list\|receive-allocate\|receive-rotate\|receive-purge\|parties ...>` | Fully covered. Plan task 3.2 (additive): `receive-addresses/purge` permanently deletes a RETIRED allocation and its counterparty binding (active → 409, unknown → 404; the party record always remains). |
...
| 29 | Stealth deposits | `deposits/eth-stealth` (+`/create-native`,`/create-erc20`,`/scan-announcements`,`/delete`,`/refresh`,`/enqueue-sweep`) — 7 routes | `operations.ts` (+ `receiving.ts` list) | `sigillum api deposits <list\|create-native\|create-erc20\|scan-announcements\|refresh\|enqueue-sweep\|delete>` | Fully covered. Gas story (plan task 2.4, additive fields, no new routes): both create routes accept `request_gas`/`gas_amount_wei_hex` (UI "Request gas from payer" checkbox + amount; CLI `--request-gas`/`--gas-amount-wei-hex`) driving EIP-5564 SHOULD-layout announcement metadata; scans auto-populate asset/amount from metadata hints (CLI `--token-address` now optional for standards-following payers); deposit rows surface `requested_gas_wei_hex`, the sponsor top-up job state (`gas_topup_job_id`/`gas_topup_job_state`), and what a `funded_needs_gas` deposit waits for. Sponsor top-ups enqueue as `eth_stealth_gas_topup` jobs (observable under row 30) with the dependent sweep's `prerequisite_job_ids`; there is deliberately no public enqueue route for them — only the deposit sweep flow emits them. Plan task 3.5 (additive): the enqueue-sweep response carries advisory `risk_findings` (`common_gas_funder`) alongside the existing `linkage_warning`. |
| 30 | Queue (observe/process) | `queue/jobs`, `queue/process`, `queue/pause`, `queue/resume` | `operations.ts` (drives fetches and process summaries, including `operator_action_required`, the pause/resume control, and the paused banner; Process Queue offers Run in background (`run_async`) and surfaces the returned operation id; `queue.ts` renders the queue view) | `sigillum api queue <list\|process\|pause\|resume>` (`process --run-async`; JSON includes per-job `state`/`last_error` and process counts) | Fully covered. `pause` latches before waiting on the active drain's mutex and is checked between jobs and immediately pre-broadcast; `resume` clears the latch only after durable policy persistence. Both flip `TreasuryPolicy.execution_paused` and emit typed gate-flip audit events. The background scheduler (plan task 1.6, `service/scheduler.rs`) advances retries/receipt-confirmation/deposit-refresh through these same paths without a client — no new routes; its status surfaces in `diagnostics` (row 6) and its advancing cycles appear under `operations` (row 34) as completed `scheduler_cycle` records. |
