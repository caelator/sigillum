...
**Counts (cross-checked against the router):** 121 route registrations
(`grep -c '\.route(' crates/sigillum-daemon/src/routes/mod.rs` → 121), which
is 122 method endpoints because `/api/treasury/parties` registers both GET
and POST. The 32 family rows below cover 121/121 registrations; each route
...
| 30 | Queue (observe/process) | `queue/jobs`, `queue/process` | `operations.ts` (drives fetches; `queue.ts` renders the queue view) | `sigillum api queue <list\|process>` | Fully covered. |
...
- Route registrations in `crates/sigillum-daemon/src/routes/mod.rs`: **121**
  (`grep -c '\.route('`). Method endpoints: **122** (`/api/treasury/parties`
  is GET+POST on one registration).
- Sum of routes across the 32 rows above: **121** — every registration
  appears in exactly one row.
