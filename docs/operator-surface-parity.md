...
| 27 | Treasury | `treasury/overview`, `treasury/policy` (+`/update`,`/enable-opt-in`), `treasury/receive-addresses` (+`/allocate`,`/rotate`), `treasury/parties` (GET+POST) (+`/update`,`/delete`) — 10 registrations | `treasury.ts` (+ `journey.ts`, `walletManager.ts`; `setup.ts` uses the atomic opt-in endpoint so onboarding cannot overwrite unrelated policy fields) | `sigillum api treasury <overview\|policy\|policy-update\|receive-list\|receive-allocate\|receive-rotate\|parties ...>` | Fully covered. The narrow opt-in mutation is a race-safe UI convenience over the same policy contract; general CLI policy updates remain available. |
