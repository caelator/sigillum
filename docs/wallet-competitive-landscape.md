...
- Scam Sniffer and adjacent Web3 security tools keep the market focused on
  malicious websites, blacklist APIs, wallet drainers, and signing warnings:
  <https://www.scamsniffer.io/>
- Hypernative-style security products show the institutional version of the same
  need: address reputation, policy enforcement, and transaction-level screening:
...
| Seed/xpub discovery | EVM scan foundation for seed/xpub profiles with gap-limit style scan over configured providers, imported receive/account/custom receive-path xpubs, plus address classifications for signer availability, watch-only status, gas availability, stranded value, approval exposure, and dormant candidates | MetaMask/Ledger/Trezor account discovery, Bitcoin xpub wallets | Scan common Ethereum derivation paths, historical receive addresses, richer last-activity windows, and configured L1/L2 chains |
| Watch-address archaeology | Ad-hoc `eth-watch` EVM address probes can inventory pasted old/client/hardware/external addresses across configured providers while preserving watch-only execution blockers. Bulk UI lines, CLI watch-address files, de-duped scans, JSON inventory report export, and a persisted daemon/CLI/UI watch-address book now cover the first Rotki-style account-import/reporting slices. | MetaMask Portfolio watched accounts, Rotki address tracking | Last-activity windows, CSV roundtrip export, address-book import/export, and signer-material reconciliation |
...
| Airdrops and rewards | Operator-configured trusted claim candidates are recorded as `airdrop` or `reward` holdings, emit claim-contract risk findings, support standard Merkle claim simulation, and remain blocked from execution | Portfolio tools, protocol claim portals | Trusted-source candidate discovery, claim-contract verification, simulation, explicit review only |
| Approvals and revokes | Bounded ERC-20 allowance probes, Permit2 allowance probes, NFT operator-approval probes, a local spender/operator risk catalog, reviewable revoke plan steps, provider-backed revoke simulation, and unsigned manifest export for approved steps | Revoke.cash, Rabby, Scam Sniffer | External spender registries, expiration-aware Permit2 scoring, direct execution policy, and queue-backed signing |
| Transaction simulation | Provider-backed preflight simulation is implemented for native sweeps, ERC-20 sweeps, NFT sweeps, ERC-20 approval revokes, Permit2 allowance revokes, NFT operator revokes, and standard Merkle claim candidates; swaps and protocol exits still need simulation coverage | Rabby, Safe tools, Tenderly-style infrastructure | Every plan step carries simulation evidence or remains blocked |
| Consolidation planning | Foundation exists: inventory, risk findings, dry-run plans, approval state, simulation state, and auditable call-manifest/Safe-batch export for approved unblocked steps | No single consumer wallet owns this end to end | Execution graph for gas top-ups, simulated revokes, claims, DeFi exits, swaps, sweeps, and treasury routing |
...
- Approved simulated plan exports give operators execution evidence they can
  move into a policy wallet without asking Sigillum to blind-sign or silently
  sweep assets.
...
the operator which value can actually be recovered. Ad-hoc `eth-watch` scans
now give pasted external addresses an explicit watch-only rung rather than
mixing them with seed/xpub profiles or signer-unknown holdings.
...
3. Extend approvals, NFTs, DeFi, and airdrops as separate adapter families with
   ...
   probes, trusted claim-candidate ingestion, and claim-contract risk findings
   are now the first slices, while metadata, spam classification, protocol exit
   adapters, and verified claim adapters remain open.
