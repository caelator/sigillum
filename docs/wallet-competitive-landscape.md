...
| Watch-address archaeology | Ad-hoc `eth-watch` EVM address probes can inventory pasted old/client/hardware/external addresses across configured providers while preserving watch-only execution blockers | MetaMask Portfolio watched accounts, Rotki address tracking | Bulk import, tags, last-activity windows, CSV/report export, and signer-material reconciliation |
