...
| Watch-address archaeology | Ad-hoc `eth-watch` EVM address probes can inventory pasted old/client/hardware/external addresses across configured providers while preserving watch-only execution blockers. Bulk UI lines, CLI watch-address files, de-duped scans, JSON inventory report export, and a persisted daemon/CLI watch-address book now cover the first Rotki-style account-import/reporting slices. | MetaMask Portfolio watched accounts, Rotki address tracking | UI address-book management, last-activity windows, CSV roundtrip export, and signer-material reconciliation |
