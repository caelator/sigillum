...
| Airdrops and rewards | Operator-configured trusted claim candidates are recorded as `airdrop` or `reward` holdings, emit claim-contract risk findings, and remain blocked in claim plans until an adapter verifies the path | Portfolio tools, protocol claim portals | Trusted-source candidate discovery, claim-contract verification, simulation, explicit review only |
