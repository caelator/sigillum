...
async fn wallet_inventory_transfer_log_cursors_scan_disjoint_ranges() {
    ...
    // A completed job is terminal: cancel conflicts, while the subsequent
    // scan still resumes its log range from the completed job's cursor.
...
async fn wallet_inventory_live_cancel_is_prompt_terminal_and_resumable() {
    ...
    assert_eq!(resume_json["job"]["status"], "running");
    assert!(resume_json["operation"].is_object());
    let resumed_job_id = resume_json["job"]["id"].as_str().unwrap().to_string();
    assert_ne!(resumed_job_id, job_id);

    let resumed_job = tokio::time::timeout(Duration::from_secs(5), async {
        loop {
            let jobs = get(&client, addr, "/api/discovery/jobs", Some(&token)).await;
            let jobs: serde_json::Value = jobs.json().await.unwrap();
            if let Some(job) = jobs["jobs"].as_array().and_then(|jobs| {
                jobs.iter()
                    .find(|job| job["id"].as_str() == Some(&resumed_job_id))
            }) {
    ...
    .await
    .expect("resumed discovery job should become terminal");
    assert_eq!(resumed_job["status"], "completed");
