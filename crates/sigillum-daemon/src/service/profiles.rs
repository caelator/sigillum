...
use sigillum_core::derive_ethereum_address_from_xpub;
...
impl SigillumService {
    ...
    pub(crate) async fn upsert_eth_xpub_wallet_profile(
        ...
        let execution_enabled = if external_receive_xpub.is_some() {
            false
        } else {
...
mod tests {
    ...
    async fn imported_xpub_profile_forces_watch_only_execution() {
        ...
        let response = service
            .upsert_eth_xpub_wallet_profile(
                Some(&session),
                xpub_upsert_request(Some(export.receive_xpub.clone())),
            )
            .await
            .unwrap();
    ...
    async fn imported_xpub_profile_rejects_invalid_receive_branch() {
        ...
        let error = service
            .upsert_eth_xpub_wallet_profile(
                Some(&session),
                xpub_upsert_request(Some("not-a-real-xpub".into())),
            )
            .await
            .unwrap_err();
