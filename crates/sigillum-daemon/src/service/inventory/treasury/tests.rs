...
use sigillum_api::{
    Counterparty, EthStealthDeposit, EvmProviderProfile, TreasuryReceiveAllocation,
    TreasuryReceiveSummary, WalletInventoryAddress,
};
...
use super::receiving::{
    RECEIVING_LINKAGE_WARNING, active_receiving_allocations, build_receiving_overview,
    hd_receiving_item, receiving_allocation_identity, receiving_refresh_work_items,
    stealth_receiving_item,
};
...
fn hd_receiving_item_uses_freshest_matching_wallet_chain_observation() {
    ...
    let allocation = receiving_allocation("alloc-base", address, RECEIVE_STATUS_ACTIVE, None, 1);
...
fn receiving_refresh_work_items_match_chain_and_preserve_provider_partitions() {
    ...
    let mainnet = receiving_allocation("alloc-mainnet", address, RECEIVE_STATUS_ACTIVE, None, 1);
